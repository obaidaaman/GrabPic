...
@user_router.get("/join-space", status_code=status.HTTP_200_OK, response_model= SpaceResponseSchema)
...
def get_images( request: Request, space_id: str,current_user= Depends(ct.is_authenticated)):
    return controller.get_images(current_user["id"], request.app.state.db, space_id)